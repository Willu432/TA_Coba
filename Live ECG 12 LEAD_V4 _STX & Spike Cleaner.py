...
# saved_model_path = "D:\EKG\Program 3 Channel\Model Gipen/modelann_nonorm2.h5" 
# loaded_model = tf.keras.models.load_model(saved_model_path)

# scaler_path = 'D:\EKG\Program 3 Channel\Model Gipen/scaler2.pkl'
...
while True:
        ...
        try:
           ...
           # Estimasi Fs seperti sebelumnya
            ...
            try:
                ...
                # 5. ST SEGMENT (ALGORITMA 100% ORIGINAL)
                if len(waves_dwt['ECG_T_Onsets']) > 1 and len(waves_dwt['ECG_R_Offsets']) > 1:
                    ST_peak_list = []
                    try:
                ...
                print(f'{lead_name} - ST Segment (ms) - Mean: {ST_avg:.2f}')
    ...
    # Model prediction section (commented out seperti original)
    # modelData = np.array([RR_avg, RR_stdev, PR_avg, QS_avg, QTc_avg, ST_avg, RS_ratio, bpm, RT_avg, PP_avg, T_Interval_avg, TP_avg, QTP_avg, QRS_Duration_avg, PT_Ratio_avg, HRV_RMSSD]).reshape(1, -1)
    # modelData_normalized = scaler.transform(modelData)
    # print(f"Input data for prediction: {modelData}")
    # try:
