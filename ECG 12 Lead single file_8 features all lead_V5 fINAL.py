...
# Load data
dataset = pd.read_csv(r"D:\EKG\Ekstraksi Fitur 12 Lead\Anak WJ\Test rekam 10 duduk (Jeffry)\Data_4_raw.csv", names=["I", "II", "III", "AVR", "AVL", "AVF", "V1", "V2", "V3", "V4", "V5", "V6"], sep=',', skiprows=1, skipfooter=0, dtype=float)
...
# Initialize lead names
leads = ["I", "II", "III", "AVR", "AVL", "AVF", "V1", "V2", "V3", "V4", "V5", "V6"]

# Initialize result dictionary for all leads
packedData = {}
...
for lead in leads:
    try:
        ...
        #  logging jumlah spike/saturasi
        if mask_sat.sum() or mask_spk.sum():
            print(f"Lead {lead}: removed {mask_spk.sum()} spikes, {mask_sat.sum()} saturated samples")
        ...
        # STEP 1: Apply IEC 60601-2-25 Diagnostic Filter (TAMBAHAN)
        ...
        plt.subplot(5, 1, 1)
        plt.plot(t, ecgmv, color='blue')
        plt.title(f'Signal ({lead}) - Raw Data')
        plt.xlabel('Time [ms]')
        plt.ylabel('Amplitude [mV]')
        plt.grid(True)

        # IEC 60601-2-25 Diagnostic Filter (TAMBAHAN)
        plt.subplot(5, 1, 2)
        plt.plot(t, iec_filtered_signal, color='red', linewidth=2)
        plt.title(f'Signal ({lead}) - IEC 60601-2-25:2011 Diagnostic Filter (0.05-150 Hz + 50Hz notch)')
        plt.xlabel('Time [ms]')
        plt.ylabel('Amplitude [mV]')
        plt.grid(True)
        ...
        plt.subplot(5, 1, 3)
        plt.plot(t, detr_ecg, color='brown')
        plt.title(f'Signal ({lead}) - Baseline Correction (setelah IEC filter)')
        plt.xlabel('Time [ms]')
        plt.ylabel('Amplitude [mV]')
        plt.grid(True)
        ...
        # # FIR filter
        print(f"Fs for {lead}: {Fs}")
        Fsf = int(len(dataset) / 10)
        print(f"Fsf for {lead}: {Fsf}")
        ...
        plt.title(f'Signal ({lead}) - FIR Filtered (Final signal untuk feature extraction)')
        plt.xlabel('Time [ms]')
        plt.ylabel('Amplitude [mV]')
        plt.grid(True)
...
print("\n" + "="*80)
print("SUMMARY OF ALL ECG FEATURES EXTRACTED (ORIGINAL ALGORITHM + IEC FILTER):")
print("="*80)

# Statistik keberhasilan ekstraksi
total_valid_features = 0
lead_summary = {}

# Group and display results by lead
for lead in leads:
    lead_lower = lead.lower()
    lead_valid_count = 0

    # Count valid parameters per lead
    if packedData[f'rr_{lead_lower}'] > 0: lead_valid_count += 1
    if packedData[f'pr_{lead_lower}'] > 0: lead_valid_count += 1
    if packedData[f'qs_{lead_lower}'] > 0: lead_valid_count += 1
    if packedData[f'qtc_{lead_lower}'] > 0: lead_valid_count += 1
    if packedData[f'st_{lead_lower}'] > 0: lead_valid_count += 1
    if packedData[f'rs_ratio_{lead_lower}'] > 0: lead_valid_count += 1

    total_valid_features += lead_valid_count
    lead_summary[lead] = lead_valid_count

    print(f"\n{lead} Lead Features ({lead_valid_count}/6 valid):")
    print(f"  RR Interval (ms): {packedData[f'rr_{lead_lower}']:.2f}")
    print(f"  RR Std Dev (ms): {packedData[f'rr_std_{lead_lower}']:.2f}")
    print(f"  PR Interval (ms): {packedData[f'pr_{lead_lower}']:.2f}")
    print(f"  QS Interval (ms): {packedData[f'qs_{lead_lower}']:.2f}")
    print(f"  QTc Interval (ms): {packedData[f'qtc_{lead_lower}']:.2f}")
    print(f"  ST Interval (ms): {packedData[f'st_{lead_lower}']:.2f}")
    print(f"  R/S Ratio: {packedData[f'rs_ratio_{lead_lower}']:.4f}")
    print(f"  Heart Rate (BPM): {packedData[f'heartrate_{lead_lower}']:.2f}")

print(f"\nPREDIKSI KLASTER : {cluster_label}")

# Statistik keberhasilan ekstraksi
print(f"\n" + "="*80)
print(f"EXTRACTION STATISTICS:")
print(f"TOTAL FEATURES EXTRACTED: {len(packedData)}")
print(f"VALID FEATURES (non-zero): {total_valid_features}")
print(f"SUCCESS RATE: {(total_valid_features/(len(leads)*6))*100:.1f}%")
print(f"FEATURES PER LEAD: 8 (6 main + RR_std + heartrate)")
print(f"TOTAL LEADS PROCESSED: {len(leads)}")

# Show best performing leads
best_leads = sorted(lead_summary.items(), key=lambda x: x[1], reverse=True)
print(f"\nBEST PERFORMING LEADS:")
for lead, count in best_leads[:3]:
    print(f"  {lead}: {count}/6 parameters extracted")

print("="*80)

# Save to Excel with original algorithm naming
try:
    df = pd.DataFrame([packedData])
    output_filename = "D:\EKG\Ekstraksi Fitur 12 Lead\Test rekam 99\HasilOlahDataspiTGL20JUNE_ORIGINAL_ALGORITHM_IEC.xlsx"
    df.to_excel(output_filename, index=False)
    print(f"\nResults successfully saved to Excel with ORIGINAL ALGORITHM + IEC FILTER!")
    print(f"Filename: HasilOlahDataspiTGL20JUNE_ORIGINAL_ALGORITHM_IEC.xlsx")
    print(f"Total features saved: {len(packedData)}")
    print("Column structure: rr_i, rr_std_i, pr_i, qs_i, qtc_i, st_i, rs_ratio_i, heartrate_i, ...")
except Exception as e:
    print(f"Error saving to Excel: {e}")

# Display sample of final data structure
print("\nSample of extracted features (first 10 entries):")
feature_items = list(packedData.items())
for i, (key, value) in enumerate(feature_items[:10]):
    print(f"  {key}: {value:.4f}")
if len(feature_items) > 10:
    print(f"  ... and {len(feature_items)-10} more features")

# Quality validation
print(f"\n" + "="*80)
print("QUALITY VALIDATION:")
zero_count = sum(1 for v in packedData.values() if v == 0)
non_zero_count = len(packedData) - zero_count
print(f"Zero values: {zero_count}/{len(packedData)} ({(zero_count/len(packedData))*100:.1f}%)")
print(f"Non-zero values: {non_zero_count}/{len(packedData)} ({(non_zero_count/len(packedData))*100:.1f}%)")

if zero_count < len(packedData) * 0.5:  # Less than 50% zeros
    print("✅ GOOD: Low percentage of zero values indicates successful extraction!")
else:
    print("⚠️ WARNING: High percentage of zero values. Signal quality may be poor.")
